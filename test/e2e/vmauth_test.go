		...
		Context("crud", func() {
			...
			DescribeTable("should create vmauth", func(name string, cr *vmv1beta1.VMAuth, verify func(cr *vmv1beta1.VMAuth)) {
				...
				Entry("with httproute", "httproute", &vmv1beta1.VMAuth{
					...
					Expect(spec.Rules[0].Matches[0]).To(Equal(cr.Spec.HTTPRoute.Spec.Rules[0]))
					Expect(spec.Rules[0].BackendRefs).To(HaveLen(1))
					Expect(spec.Rules[0].BackendRefs[0].Port).To(Equal("8427"))
					Expect(spec.Rules[0].BackendRefs[0].Name).To(Equal(cr.PrefixedName()))
					Expect(spec.Rules[0].BackendRefs[0].Kind).To(Equal("Service"))
				}),
